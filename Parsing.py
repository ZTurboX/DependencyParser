    ...
    def predict(self, parsers):
        x = [
            feature.create_features(
                p.stack, p.buffer, p.dep,
                self.dataset[self.sentence2id[id(p.sentence)]])
            for p in parsers
        ]
        ...
        predict = np.argmax(
            predict_logits + 10000 * np.array(l).astype('float32'), 1)
        predict = [
            "S" if p == 2 else ("L" if p == 0 else "R") for p in predict
        ]
        return predict
